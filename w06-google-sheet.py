...
# Page configuration
st.set_page_config(page_title="ระบบจัดการร้านอาหาร",page_icon="🕹️",layout="wide")
...
def show_dialogAddProd():
    ...
    brand = st.selectbox("แบรนด์", ["SE-ED", "ENFANT", "IKEA", "LEGO"])
...
def show_dialogEditProd(row_idx,product):
    ...
    if product:
        with st.form(key=f"edit_form_{product['id']}"):
            ...
            edit_brand = st.selectbox("แบรนด์", ["SE-ED", "ENFANT", "IKEA", "LEGO"], index=["SE-ED", "ENFANT", "IKEA", "LEGO"].index(product["brand"]))
